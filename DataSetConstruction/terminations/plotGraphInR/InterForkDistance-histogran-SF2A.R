...
NonSinc   <- read.csv2('StatsLengthRepliconNonSinc-Term.csv')
NonSinc
# Renaming to columns
...
B <- ggplot(df, aes(x = Length, y = Count, fill = as.factor(Length))) +
  ...
  theme_classic() +
  theme(axis.text.x = element_text(size = 12, angle = 90, vjust = -0.01), 
        axis.title.y = element_text(size = 12, angle = 90, vjust = -0.00001),
        axis.title.x = element_text(size = 12, vjust = -0.00001), 
        axis.text = element_text(size = 12, colour = "Black")) +
  coord_cartesian(ylim = c(0, 1000)) +
  scale_y_continuous(breaks = seq(0, 1000, by = 100), expand = expansion(mult = c(0.005, 0.05))) +
  theme(legend.position = "none") +
  labs(x = "Inter-fork distance (bp)", y = "Count", fill = "Length")  
  
B

ggsave("LenghtPutativeTermination.pdf", B, height=9, width=9, dpi=1200, , units = "cm")
# Save plot A in paper file
setwd("~/Documents/paper_GenomeVariability/figures/Original/datasetConstrutionandClassification-D-NascentTermination")
ggsave("LenghtPutativeTermination.pdf", B,height=9, width=18, dpi=1200, , units = "cm")

...
# =============================
matrix_files <- list(
  LessFrequent = "matrix-lessfrequent-mergedOrigins.gz",
  MoreFrequent = "matrix-moreFrequent-mergedOrigins.gz"
)
...
# =============================
plot_profile <- function(plot_data, title_text, y_min = 0, y_max = 5, x_label_color = rgb(0, 0, 0, maxColorValue = 255)) {
  ggplot(plot_data, aes(x = position, y = mean_signal)) +
    geom_line(color = rgb(255, 204, 0, maxColorValue = 255), size = 1.5) +
...
plot_A <- plot_profile(
  ...
  "origins",
  x_label_color = rgb(44, 123, 182, maxColorValue = 255)
) +
  annotate("text", x = 0, y = Inf, label = "<10", hjust = 0, vjust = 2,
           color = rgb(44, 123, 182, maxColorValue = 255), size = 6)

plot_B <- plot_profile(
  plot_data_list$MoreFrequent,
  "mini-initiation zones",
  x_label_color = rgb(215, 25, 28, maxColorValue = 255)
) +
  annotate("text", x = 0, y = Inf, label = ">=10", hjust = 0, vjust = 2,
           color = rgb(215, 25, 28, maxColorValue = 255), size = 6)
...
ggsave("originsandinitiationzones.pdf", final_plot, height = 9, width = 9, dpi = 1200, units = "cm")
...
ggsave("originsandinitiationzones.pdf", final_plot, height = 7, width = 16, dpi = 1200, units = "cm")
